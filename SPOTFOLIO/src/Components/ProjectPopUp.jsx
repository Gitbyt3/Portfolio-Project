    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black/60 backdrop-blur-md font-[Verdana]" onClick={onClose}>
      <div className="bg-zinc-900 border border-white/10 rounded-2xl p-8 relative w-fit max-w-[80%] max-h-[75%] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mb-6 rounded-lg overflow-hidden">
          <img src={station.image} alt={station.title} className="w-[60%] rounded-2xl h-auto mx-auto object-contain" />
        </div>
        
        <div className="flex items-center justify-between gap-4 mb-4">
          <h2 className="text-2xl font-bold text-white">{station.title}</h2>
          <a href={station.githubLink} target="_blank" className="text-zinc-500 hover:text-white mr-4" title="GitHub Link">
        ...
        <button className="mt-8 w-full block mx-auto py-4 bg-white text-black rounded-lg font-bold hover:bg-zinc-200 text-xs" onClick={onClose}>
